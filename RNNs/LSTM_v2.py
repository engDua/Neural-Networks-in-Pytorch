...
pred_dir = pred_folder ### path to folder where the predicted feats will be stored
...
source_folder=<path to input folder>  ### input files
k = 0
os.chdir(source_folder)
files = sorted(os.listdir('.'))
for file in files:
  if k < 800: ### number of files to be used
     k = k+1
...
target_folder =<path to output folder> ### output files
...
hidden = 512  ## assign the number of hidden units
...
for i in range(0,30):
  ...
  for in_batch, output, lengths in train_loader:
    ...
    loss.backward()
    optimizer.step()
  print("loss is:", total_loss/4) # divided by no. of examples, batch_size here

torch.save(model, <path to save model>)  #### path to save the model
...
##    in_batch, output, lengths = sort_batch(in_batch, output, lengths)
    ...
    Model = torch.load(<path to load the model from>)   ### path to load the model from
    print("loading model")
